...
public class DEC extends Commands {
    ...
    public DEC(MainPanel main) {
        super(main);
    }

    @Override
    public void execute() {
        String regex0 = MnemonicsMap.getInstance().getMnemonicsMap().get("DEC").getRegexList().get(0).getRegex(true);
        String regex1 = MnemonicsMap.getInstance().getMnemonicsMap().get("DEC").getRegexList().get(1).getRegex(true);
