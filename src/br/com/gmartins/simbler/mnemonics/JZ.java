 ...
 * @author Guilherme
 */
public class JZ extends Commands {

    public JZ(MainPanel main) {
        super(main);
    }

    @Override
    public void execute() {
        final String workingCommand = "JZ";
        String regex0 = MnemonicsMap.getInstance().getMnemonicsMap().get(workingCommand).getRegexList().get(0).getRegex(true);

        // MATCHES JZ 10, JNO 500
        // REGEX[0] - "JZ [0-9]{1,9}"};

        if (RegexMatcher.matches(regex0, getCurrentInstruction().getLineCommand())) {
            // Pega o valor do registrador 
            int value = getRegisters().getZero().toInteger();
            // Se o valor do registrador estiver em "1", indica que a última operação resultou em 0. Então pula
