 ...
 * @author Guilherme
 */
public class JS extends Commands {

    public JS(MainPanel main) {
        super(main);
    }

    @Override
    public void execute() {
        final String workingCommand = "JS";
        String regex0 = MnemonicsMap.getInstance().getMnemonicsMap().get(workingCommand).getRegexList().get(0).getRegex(true);

        // MATCHES JS 10, JNO 500
        // REGEX[0] - "JS [0-9]{1,9}"};
        ...
        if (RegexMatcher.matches(regex0, getCurrentInstruction().getLineCommand())) {
            ...
            // Se o valor do registrador estiver em "1", indica que a última operação resultou uma operação negativa.
            if (value == 1) {
